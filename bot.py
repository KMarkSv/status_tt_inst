...
# === НАСТРОЙКИ ===
TOKEN = '1724022158:AAFyHkotXpwtp9mHacmQMCC-RTjRDx_jOZ0'

INSTAGRAM_USERNAME = 'vasiakushv'
INSTAGRAM_PASSWORD = 'kmskmskms'
PROXY = "http://markkukiko:La7gwGRdgQ@ip:port"
...
async def instagram_check(username):
    ...
    try:
        ...
        return "ПРИВАТНЫЙ" if profile.is_private else "ОТКРЫТЫЙ"
    except Exception as e:
...
async def cmd_start(message: Message):
    text = (
        "Привет! Я могу отслеживать статус аккаунтов Instagram и TikTok.\n\n"
        "Используй команды:\n"
...
async def ins_delete(message: Message):
    command_parts = message.text.split()
...
async def instagram_receive_username(message: Message):
    ...
    if username in accounts:
        ...
        if status:
            accounts[username] = {'status': status, 'chat_id': message.chat.id}
            save_accounts(INSTAGRAM_FILE, accounts)
            emoji = '✅' if status == 'ПРИВАТНЫЙ' else '🆘'
            await message.answer(f"Instagram аккаунт @{username} добавлен.\nТекущий статус: {emoji}{status}")
        else:
            await message.answer(f"Не удалось получить информацию по Instagram аккаунту @{username}.")
...
async def tiktok_receive_username(message: Message):
    ...
    if username in accounts:
        ...
        if status:
            ...
            save_accounts(TIKTOK_FILE, accounts)
            emoji = '✅' if status == 'ПРИВАТНЫЙ' else '🆘'
            await message.answer(f"TikTok аккаунт @{username} добавлен.\nТекущий статус: {emoji}{status}")
        else:
            await message.answer(f"Не удалось получить информацию по TikTok аккаунту @{username}.")
    ...
    while True:
        ...
        for username, info in accounts.items():
            ...
            if current_status and current_status != info['status']:
                emoji_old = '✅' if info['status'] == 'ПРИВАТНЫЙ' else '🆘'
                emoji_new = '✅' if current_status == 'ПРИВАТНЫЙ' else '🆘'
                chat_id = info['chat_id']
                await bot.send_message(chat_id,
                    f"🔔 Изменился статус Instagram аккаунта @{username}:\n"
                    f"Было: {emoji_old}{info['status']}\n"
                    f"Стало: {emoji_new}{current_status}")
                accounts[username]['status'] = current_status
                save_accounts(INSTAGRAM_FILE, accounts)
        await asyncio.sleep(10 * 60 * 60)  # 10 часов

async def periodic_tiktok_check():
    await asyncio.sleep(10 * 60)  # смещение
    while True:
        accounts = load_accounts(TIKTOK_FILE)
        for username, info in accounts.items():
...
async def info(message: Message):
    accounts = load_accounts(INSTAGRAM_FILE)
    for username, info in accounts.items():
        emoji = '✅' if info['status'] == 'ПРИВАТНЫЙ' else '🆘'
        await message.answer(f"Instagram аккаунт @{username}\nТекущий статус: {emoji}{info['status']}")
...
# =============== ЗАПУСК ===============
async def main():
    dp.message.register(instagram_receive_username, F.text, lambda m: False)
